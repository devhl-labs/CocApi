...
namespace devhl.CocApi
{
    ...
    {
        public WebResponse(CocApi cocApi, CocApiConfiguration cfg, IEnumerable<string> tokens)
        {
            _cocApi = cocApi;
            ...
            {
                TokenObject tokenObject = new TokenObject(cocApi, token, _config.TokenTimeOut);
        ...
        public async Task CacheAsync(string path, string json, EndPoint endPoint, DateTime serverExpiration)
        {
            ...
            try
            {
                Cache? cache = await _cocApi.SqlWriter.Select<Cache>()
                                                    .Where(c => c.Path == path)
        ...
        public async Task<IDownloadable?> GetDownloadableAsync<TValue>(string encodedUrl, TokenObject token, CancellationToken cancellationToken) where TValue : IDownloadable /*, new()*/
        {
        ...
        internal async Task<TokenObject> GetTokenAsync()
        {
        ...
        private async Task<HttpResponseMessage> GetHttpResponseAsync(EndPoint endPoint, string encodedUrl, TokenObject token, CancellationToken cancellationToken)
        {
        ...
        private IDownloadable? UnSuccessfulResponse(HttpResponseMessage response, string encodedUrl, EndPoint endPoint, TokenObject token)
        {
            string responseText = response.Content.ReadAsStringAsync().Result;
