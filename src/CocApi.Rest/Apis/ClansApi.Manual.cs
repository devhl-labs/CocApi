...
namespace CocApi.Rest.Apis
{
    public partial class ClansApi
    {
        ...
        partial void AfterFetchClan(ref bool suppressDefaultLog, ApiResponse<Clan> apiResponseLocalVar, string clanTag)
        {
            ...
            Logger.LogInformation("{0,-9} | {1} | {3}", (apiResponseLocalVar.DownloadedAt - apiResponseLocalVar.RequestedAt).TotalSeconds, apiResponseLocalVar.StatusCode, apiResponseLocalVar.Path.Replace("{clanTag}", clanTag));
        }
        ...
        {
            suppressDefaultLog = true;
            Logger.LogInformation("{0,-9} | {1} | {3}", (apiResponseLocalVar.DownloadedAt - apiResponseLocalVar.RequestedAt).TotalSeconds, apiResponseLocalVar.StatusCode, apiResponseLocalVar.Path.Replace("{warTag}", warTag));
        }
