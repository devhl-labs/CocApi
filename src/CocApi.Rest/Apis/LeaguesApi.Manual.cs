...
public partial class LeaguesApi
{
    partial void AfterFetchBuilderBaseLeague(ref bool suppressDefaultLog, IFetchBuilderBaseLeagueApiResponse apiResponseLocalVar, string leagueId)
    {
        ...
        Logger.LogTrace(RestLogEvents.ApiRequestCompleted, "{elapsed,-9} | {status} | {url}", (apiResponseLocalVar.DownloadedAt - apiResponseLocalVar.RequestedAt).TotalSeconds, apiResponseLocalVar.StatusCode, apiResponseLocalVar.Path.Replace("{leagueId}", leagueId));
    }
