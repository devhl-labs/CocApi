...
public partial class PlayersApi
{
    partial void OnErrorFetchPlayer(ref bool suppressDefaultLogLocalVar, Exception exceptionLocalVar, string pathFormatLocalVar, string pathLocalVar, string playerTag)
    {
        ...
        Logger.LogError(RestLogEvents.ApiRequestFailed, exceptionLocalVar, "There was an error fetching the player for playerTag: {playerTag}", playerTag);
    }
    ...
    {
        suppressDefaultLogLocalVar = true;
        Logger.LogError(RestLogEvents.ApiRequestFailed, exceptionLocalVar, "There was an error fetching the token for playerTag: {playerTag}", playerTag);
    }
