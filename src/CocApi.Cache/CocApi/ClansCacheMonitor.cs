...
{
    public sealed class ClansCacheMonitor
    {
        private readonly CachedContext _dbContext1;
        ...
        public ClansCacheMonitor(CocApiClient cocApi, CocApiConfiguration cocApiConfiguration, /*IServiceProvider serviceProvider,*/ ClansApi clansApi, CachedContext dbContext)
        {
            _cocApi = cocApi;
        ...
        private void UpdateClans()
        {
            Task.Run(async () =>
            {
                try
                {
                    ...
                    _cocApi.OnLog(new LogEventArgs(nameof(ClansCacheMonitor), nameof(UpdateClans), LogLevel.Information));
                    ...
                    while (!StopUpdatingClansRequested)
                    {
                        ...
                        List<CachedClan> cachedClans = await _dbContext.Clans.Where(v =>
                        ...
                        for (int i = 0; i < cachedClans.Count; i++)
                            tasks.Add(UpdateEntireClanAsync(cachedClans[i], _dbContext));
                ...
                {
                    _cocApi.OnLog(new ExceptionEventArgs(nameof(ClansCacheMonitor), nameof(UpdateClans), e));
        ...
        private void UpdateWars()
        {
            Task.Run(async () =>
            {
                try
                {
                    ...
                    _cocApi.OnLog(new LogEventArgs(nameof(ClansCacheMonitor), nameof(UpdateWars), LogLevel.Information));
                ...
                {
                    _cocApi.OnLog(new ExceptionEventArgs(nameof(ClansCacheMonitor), nameof(UpdateWars), e));
        ...
        public async Task StopAsync()
        {
            ...
            _cocApi.OnLog(new LogEventArgs(nameof(ClansCacheMonitor), nameof(StopAsync), LogLevel.Information));
