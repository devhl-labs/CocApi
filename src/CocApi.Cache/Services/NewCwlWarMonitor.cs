...
namespace CocApi.Cache
{
    ...
    public sealed class NewCwlWarMonitor : PerpetualMonitor<NewCwlWarMonitor>
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                try
                {
                    apiResponse = await _clansApi.FetchClanWarLeagueWarResponseAsync(kvp.Key, cancellationToken).ConfigureAwait(false);
                }
                catch (Exception)
                {
                }
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                TimeSpan timeToLive = await _ttl.TimeToLiveOrDefaultAsync(war).ConfigureAwait(false);
