...
public sealed class ClanWarService : ServiceBase
{
    ...
    protected override async Task ExecuteScheduledTaskAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "ClanWarService cycle | Fetched={Fetched} | Updated={Updated} | LockSkips={LockSkips} | SaveMs={SaveMs} | TotalMs={TotalMs} | TP Worker={UsedWorker}/{MinWorker}/{MaxWorker} avail={AvailWorker} | TP IOCP={UsedIocp}/{MinIocp}/{MaxIocp} avail={AvailIocp} | TP Threads={ThreadCount} | TP Pending={Pending}",
            ...
            if (cycleSw.ElapsedMilliseconds > 5000)
                _logger.LogInformation(
                    "ClanWarService cycle slow | Fetched={Fetched} | Updated={Updated} | LockSkips={LockSkips} | SaveMs={SaveMs} | TotalMs={TotalMs}",
                    cachedClans.Count, updatingTags.Count, lockSkips, totalSaveMs, cycleSw.ElapsedMilliseconds);

            if (availWorker < 10 || pendingItems > 100)
                _logger.LogWarning(
                    "ClanWarService thread pool pressure | AvailWorker={AvailWorker} | Pending={Pending} | ThreadCount={ThreadCount}",
